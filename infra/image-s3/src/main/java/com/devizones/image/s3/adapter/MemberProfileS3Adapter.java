...
public class MemberProfileS3Adapter implements MemberProfileImagePort {
    private final S3Properties s3Properties;
    private final AmazonS3Client amazonS3Client;
    @Override
    public void upload(String fileName, InputStream stream) {
        try (InputStream inputStream = stream) {
            String path = s3Properties.getProfilePrefix() + fileName;
    ...
    public void remove(String fileName) {
        amazonS3Client.deleteObject(new DeleteObjectRequest(s3Properties.getBucket(), fileName));
    }

    @Override
    public boolean existImage(String fileName) {
        return amazonS3Client.doesObjectExist(s3Properties.getBucket(), fileName);
    }
}
